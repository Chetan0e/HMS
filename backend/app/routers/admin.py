...
async def get_owner_analytics(current_user: dict = Depends(require_roles(["OWNER", "MANAGER", "ADMIN"]))):
    ...
    total_properties = await db.properties.count_documents({"owner_id": owner_id})
    total_rooms = await db.rooms.count_documents({"property_id": {"$in": [str(p["_id"]) async for p in db.properties.find({"owner_id": owner_id})]}})
    
    beds_cursor = db.beds.find({"property_id": {"$in": [str(p["_id"]) async for p in db.properties.find({"owner_id": owner_id})]}})
    total_beds = 0
    occupied_beds = 0
    available_beds = 0
    ...
    upcoming_visits = await db.visits.count_documents({"owner_id": owner_id, "status": "PENDING"})
    ...
    return {
        ...
        "occupancy_trend": [
            ...
            {"month": "Jun", "occupancy": round((occupied_beds / total_beds * 100), 1) if total_beds > 0 else 90}
        ]
    }
